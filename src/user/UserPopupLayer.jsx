  ...
  return (
    <ModalPortal className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/55 px-3 py-5 backdrop-blur-sm sm:px-6">
      ...
      >
        ...
        <div className="shrink-0 border-t border-slate-100 bg-slate-50 px-3 py-3 sm:px-6 sm:py-3">
          <div className="space-y-2 sm:hidden">
            <div className="flex min-w-0 items-center gap-2">
              {hasMultiple ? (
                <div className="flex shrink-0 items-center gap-1">
                  <button type="button" onClick={() => move(-1)} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 transition hover:border-orange-300 hover:text-orange-600" aria-label="이전 팝업"><ChevronLeft size={17} /></button>
                  <span className="min-w-9 shrink-0 text-center text-[11px] font-bold text-slate-600">{activeIndex + 1} / {visibleUserPopups.length}</span>
                  <button type="button" onClick={() => move(1)} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 transition hover:border-orange-300 hover:text-orange-600" aria-label="다음 팝업"><ChevronRight size={17} /></button>
                </div>
              ) : null}

              <label className="inline-flex shrink-0 cursor-pointer items-center gap-1 whitespace-nowrap text-[11px] font-semibold text-slate-600">
                <input
                  type="checkbox"
              ...
              <select
                ...
                className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-2 py-2.5 text-[11px] font-semibold text-slate-600 outline-none transition focus:border-orange-400 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
            ...
            <div className="flex items-center justify-end gap-2">
              {hasMultiple ? (
                <button
                  ...
                  className="shrink-0 rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-xs font-bold text-slate-700 transition hover:border-slate-400 hover:bg-slate-100"
              ...
              <button
                type="button"
                onClick={handleClose}
                className="shrink-0 rounded-lg bg-orange-500 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-orange-600"
              >
                닫기
              </button>
            </div>
          ...
          <div className="hidden items-center gap-3 sm:flex">
            {hasMultiple && (
              <div className="flex shrink-0 items-center gap-1">
                ...
                <span className="min-w-10 shrink-0 text-center text-xs font-bold text-slate-600">{activeIndex + 1} / {visibleUserPopups.length}</span>
            ...
            <div className="ml-auto flex min-w-0 flex-nowrap items-center gap-2">
              <label className="inline-flex shrink-0 cursor-pointer items-center gap-1.5 whitespace-nowrap text-xs font-semibold text-slate-600">
                <input
                  type="checkbox"
                  checked={doNotShowAgain}
                  onChange={(event) => setDoNotShowAgain(event.target.checked)}
              ...
              <select
                ...
                onChange={(event) => setDismissDuration(event.target.value)}
                disabled={!doNotShowAgain}
                className="shrink-0 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold text-slate-600 outline-none transition focus:border-orange-400 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                aria-label="팝업 다시 보지 않기 기간"
              >
                <option value="session">현재 탭 동안</option>
                <option value="sevenDays">7일간</option>
              ...
              {hasMultiple && (
                <button
                  type="button"
                  onClick={handleCloseAll}
                  className="shrink-0 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition hover:border-slate-400 hover:bg-slate-100"
                  title="현재 노출 중인 팝업 모두 닫기"
                >
                  모두 닫기
                </button>
              ...
              <button
                type="button"
                onClick={handleClose}
                className="shrink-0 rounded-lg bg-orange-500 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-orange-600"
              >
                닫기
              </button>
            </div>
