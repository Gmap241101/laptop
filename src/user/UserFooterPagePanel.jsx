...
export default function UserFooterPagePanel({ ctx }) {
  ...
  const pageType = getFooterPageType(selectedFooterPage);
    ...
    return (
      <Card className="overflow-hidden border-slate-200 bg-white shadow-sm">
        ...
        <CardContent className="px-6 py-14 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">이 메뉴에는 클릭 동작이나 상세 페이지가 설정되어 있지 않습니다.</p>
  ...
  if (pageType === 'link') {
    ...
    const openInNewTab = selectedFooterPage.openInNewTab !== false;
    ...
    return (
      <Card className="overflow-hidden border-slate-200 bg-white shadow-sm">
        ...
        <CardContent className="px-6 py-14 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            아래 버튼을 누르면 {openInNewTab ? '새 탭' : '현재 탭'}에서 링크를 엽니다.
          </p>
