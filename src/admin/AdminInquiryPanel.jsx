        ...
        <>
          ...
          {detail ? (
            ...
            <>
              ...
              <div className="grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-[1fr_auto_1fr] sm:items-center"><div className="text-[11px] text-slate-500 sm:justify-self-start">검색 결과 {totalCount}건 · {page} / {totalPages}페이지</div><PaginationControls className="sm:justify-self-center" currentPage={page} totalPages={totalPages} onPageChange={(nextPage) => loadList({ targetPage: nextPage })} /><div className="flex flex-wrap gap-2 sm:justify-self-end"><Button type="button" variant="outline" onClick={() => setCategoryOpen(true)}>카테고리 관리</Button><Button type="button" variant="outline" onClick={() => setSettingsOpen(true)}>문의 설정</Button></div></div>
            </>
          )}
        </>
      )}
