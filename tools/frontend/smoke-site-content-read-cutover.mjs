...
for (const marker of ['Clerk Staging Test · Phase 28', 'Phase 24 site shell content PostgreSQL read + write-through', 'Site content 전체 동기화', 'Site content PostgreSQL enabled count', 'Site content Firestore server enabled count', "top: '184px'"]) assert.ok(diagnostics.includes(marker), `missing Phase 24 diagnostics marker: ${marker}`);
