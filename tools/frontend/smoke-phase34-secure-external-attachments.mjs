...
assert.match(boardController, /requestAdminNoticePost/);
assert.match(boardController, /requestAdminFaqPost/);
assert.match(boardController, /targetUrl: attachment\?\.targetUrl \|\| ''/);
assert.doesNotMatch(boardController, /\{ \.\.\.attachment, targetUrl: '' \}/, 'administrator board editor must not blank the stored external source URL');
assert.match(boardCutover, /\/api\/admin\/boards\/notice\/posts\/\$\{encodeURIComponent\(trim\(postId\)\)\}/);
assert.match(boardCutover, /\/api\/admin\/boards\/faq\/posts\/\$\{encodeURIComponent\(trim\(postId\)\)\}/);
assert.match(boardCutover, /Authorization: `Bearer \$\{clerkToken\}`/);
...
assert.match(adminInquiry, /targetUrl: attachment\?\.targetUrl \|\| ''/);
assert.doesNotMatch(adminInquiry, /answer\.attachments\.map\(\(attachment\) => \(\{ \.\.\.attachment, targetUrl: '' \}\)\)/);
