...
const dashboardDocRef = doc(firestoreDb, 'laptopRentalDashboard', 'v2');
...
function normalizeDashboardData(source) {
  const parsed = { ...initialData, ...(source || {}) };
...
function App() {
  ...
  const initialLocalDataRef = useRef(data);
  ...
  // Firebase Firestore 문서와 브라우저 로컬 캐시를 실시간 동기화
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          setData(normalizeDashboardData(snapshot.data()));
        } else {
          isRemoteUpdateRef.current = true;
          setDoc(dashboardDocRef, initialLocalDataRef.current).catch((error) => {
            console.error('Firebase initial save error:', error);
          });
        }
        setFirebaseReady(true);
      },
      (error) => {
        console.error('Firebase synchronization error:', error);
        setFirebaseReady(true);
      }
    );
  ...
  useEffect(() => {
    ...
    setDoc(dashboardDocRef, data, { merge: true }).catch((error) => {
