...
const run = async () => {
  ...
  try {
    ...
    await client.query(
      "SELECT pg_advisory_xact_lock(hashtext('dept-laptop-rental-schema-migrations'))",
    );
    await client.query(`
    ...
    const applied = new Map(
      appliedResult.rows.map((row) => [row.name, String(row.checksum).trim()]),
    );
    ...
    for (const migration of migrations) {
      ...
      if (previousChecksum) {
        if (previousChecksum !== migration.checksum) {
          throw new Error(
            `Migration checksum mismatch for ${migration.name}; never edit an applied migration.`,
          );
        }
